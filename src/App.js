...
class App extends Component {
  ...
  setTimer = () => {
    const {initialTimerSeconds} = this.state

    if (initialTimerSeconds > 0) {
      this.setState(prevState => ({
        initialTimerSeconds: prevState.initialTimerSeconds - 1,
      }))
    } else {
  ...
  onChangeImageButton = id => {
    ...
    const isMatching = randomImgId === id
    ...
    this.setState({
      randomImgId: imagesList[imageIndex].id,
      randomImgUrl: imagesList[imageIndex].imageUrl,
    })
  }
  ...
  render() {
    ...
    const getFilteredList = this.getFilteredImages()
    ...
    return (
      <div className="app-container">
        ...
        <div className="images-container">
          {isGameInprogress ? (
            <>
              <img src={randomImgUrl} alt="match" className="random-image" />
              ...
              <ul className="images-list">
                {getFilteredList.map(eachImage => (
                  <ImagesList
                    key={eachImage.id}
                    imageDetails={eachImage}
                    onChangeImageButton={this.onChangeImageButton}
                  />
                ))}
              </ul>
            </>
            ...
            <div className="game-over-container">
              ...
              />
              <p className="your-score-name">YOUR SCORE</p>
              <h1 className="your-score">{initialScore}</h1>
              <button
